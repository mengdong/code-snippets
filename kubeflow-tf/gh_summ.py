...
def gh_summ(  #pylint: disable=unused-argument
  ...
  worker_gpu: dsl.PipelineParam = dsl.PipelineParam(name='worker_gpu', value='NUMBER_of_GPU_HERE'),
  ...
  #     name='transform',
  #     image='TENSORFLOW-WITH_TFTRT',
  #     arguments=["--worker_gpu", worker_gpu,
  #         "--model-dir", '%s/%s/model_output' % (working_dir, '{{workflow.name}}'),
  #         "--output-dir", '%s/%s/tftrt_output' % (working_dir, '{{workflow.name}}')],
  #     file_outputs={'output': '/tmp/output'}
  #     )
  # transform.after(train)
